    ...
    <footer className="border-t border-border bg-surface-elevated mt-24">
      <div className="mx-auto max-w-7xl px-6 lg:px-10 py-16 grid gap-12 md:grid-cols-2 lg:grid-cols-4">
        <div className="lg:col-span-2">
          <div className="flex items-center gap-2 mb-4">
            <div className=" w-10">
              <img src={s?.logo_url} className="w-full object-contain" />
            </div>
            <span className="font-display text-xl text-ink">
              {s?.church_name ?? "Grace Cathedral"}
            </span>
          </div>
          <p className="text-ink-muted max-w-md leading-relaxed">{s?.tagline}</p>
          <div className="flex gap-3 mt-6">
            {[Facebook, Youtube, Instagram].map((Icon, i) => (
              <a
                key={i}
                href="#"
                className="h-10 w-10 inline-flex items-center justify-center rounded-full border border-border text-ink-muted hover:text-primary hover:border-primary transition-colors"
              >
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm">
            {[
              ...
              <li key={to}>
                <Link to={to} className="text-ink-muted hover:text-primary">
                  {label}
                </Link>
              </li>
            ))}
      ...
      <div className="border-t border-border">
        <div className="mx-auto max-w-7xl px-6 lg:px-10 py-6 flex flex-col sm:flex-row justify-between gap-2 text-xs text-ink-muted">
          <span>
            © {new Date().getFullYear()} {s?.church_name ?? "Grace Cathedral"}. All rights reserved.
          </span>
