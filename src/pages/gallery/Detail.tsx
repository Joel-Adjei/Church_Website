...
export default function GalleryDetail() {
  ...
  const { data: program, isLoading, error } = useGalleryById(id);
  ...
  return (
    <article className="mx-auto max-w-7xl px-6 lg:px-10 py-12 md:py-20">
      ...
      <div className="max-w-3xl mb-12">
        ...
        {program.venue && <p className="mt-2 text-sm text-accent font-medium uppercase tracking-wider">{program.venue}</p>}
        <p className="mt-4 text-lg text-ink-muted leading-relaxed">{program.description}</p>
      </div>

      <div className="columns-1 sm:columns-2 lg:columns-3 gap-4 [&>*]:mb-4">
        {program.images.map((img, i) => (
          <button key={img.id} onClick={() => setActive(i)} className="block w-full overflow-hidden rounded-xl group">
            <img
              src={img.image}
      ...
      {active !== null && (
        ...
        >
          <button
            ...
            className="absolute top-6 right-6 h-12 w-12 inline-flex items-center justify-center rounded-full bg-primary-foreground/10 text-primary-foreground hover:bg-primary-foreground/20"
            aria-label="Close"
          >
            <X className="h-5 w-5" />
          </button>
          <button
            ...
            className="absolute left-6 h-12 w-12 inline-flex items-center justify-center rounded-full bg-primary-foreground/10 text-primary-foreground hover:bg-primary-foreground/20"
            aria-label="Previous"
          >
          ...
          <button
            ...
            className="absolute right-6 h-12 w-12 inline-flex items-center justify-center rounded-full bg-primary-foreground/10 text-primary-foreground hover:bg-primary-foreground/20"
            aria-label="Next"
          >
            <ChevronRight className="h-6 w-6" />
          </button>
          <img
            src={program.images[active].image}
            alt={program.images[active].title || ""}
