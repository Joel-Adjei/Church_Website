...
import { ArrowRight, Calendar, Clock, MapPin, Radio, PlayCircle, HandHeart, Users, BookOpen, Heart, Play } from "lucide-react";
...
const nextSteps = [
  ...
  { icon: Users, title: "Find a group", desc: "Belong to a small community midweek.", to: "/contact" },
  { icon: BookOpen, title: "Listen in", desc: "Catch up on recent sermons & series.", to: "/sermons" },
  { icon: HandHeart, title: "Serve & give", desc: "Use your gifts to bless the city.", to: "/contact" },
];

const testimonials = [
  { quote: "Grace Cathedral became home the moment we walked in. The teaching is honest, the people are warm, and our kids love it.", name: "The Okafor family", role: "Members since 2019" },
  { quote: "I came in skeptical and stayed because no one tried to package faith as easy. They just made room for my questions.", name: "Jordan M.", role: "First-time guest, now baptized" },
  { quote: "Serving with the food pantry team showed me what church can be in a city.", name: "Priya S.", role: "Mercy ministries volunteer" },
];
  ...
  return (
    <>
      <section className="relative isolate overflow-hidden">
        <div className="absolute inset-0 -z-10 bg-cover bg-center" style={{ backgroundImage: `url(${settings.bannerImageUrl})` }} />
        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-10 py-24 md:py-40">
          <div className="max-w-3xl">
            <span className="inline-block text-xs uppercase tracking-[0.25em] text-accent font-semibold mb-5">Welcome home</span>
            <h1 className="font-display text-5xl md:text-7xl lg:text-8xl text-primary-foreground leading-[1.02]">{settings.churchName}</h1>
            <p className="mt-6 text-lg md:text-xl text-primary-foreground/85 max-w-xl leading-relaxed">{settings.tagline}</p>
            <div className="mt-10 flex flex-wrap gap-3">
              <Button asChild size="lg" className="gap-2"><Link to="/live"><Radio className="h-4 w-4" /> Watch Live</Link></Button>
        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-10 py-10 grid gap-6 md:grid-cols-3">
          {settings.serviceTimes.map((s, i) => (
            <div key={i} className="flex items-start gap-3">
              ...
              <div>
                <div className="text-xs uppercase tracking-wider text-ink-muted">Service {i + 1}</div>
        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-10 py-16 md:py-20">
          ...
          <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {nextSteps.map((s) => {
              const Icon = s.icon;
              return (
                <Link key={s.title} to={s.to} className="group p-6 rounded-2xl border border-border bg-card hover:shadow-elevated transition-shadow">
      ...
      <section className="mx-auto max-w-7xl px-6 lg:px-10 py-20 md:py-28">
        <div className="grid gap-12 lg:grid-cols-2 lg:gap-16 items-center">
          <div>
            <SectionHeading eyebrow="Our story" title="Take two minutes to meet us." description="A short film about who we are, the people we're for, and the hope that holds it all together." />
          ...
          >
            <img src={STORY_THUMB} alt="Our story" className="aspect-[16/10] w-full object-cover transition-transform duration-700 group-hover:scale-105" />
      ...
      <Dialog open={videoOpen} onOpenChange={setVideoOpen}>
        <DialogContent className="max-w-4xl p-0 bg-black border-0 overflow-hidden">
          <DialogHeader className="sr-only"><DialogTitle>Our story</DialogTitle></DialogHeader>
      ...
      <section className="bg-surface-elevated border-y border-border">
        <div className="mx-auto max-w-7xl px-6 lg:px-10 py-20 md:py-28">
          <div className="flex items-end justify-between mb-12 gap-6 flex-wrap">
            ...
            <Link to="/events" className="text-sm font-medium text-primary hover:text-accent inline-flex items-center gap-1">All events <ArrowRight className="h-4 w-4" /></Link>
          </div>
      ...
      <section className="mx-auto max-w-7xl px-6 lg:px-10 py-20 md:py-28">
        <div className="flex items-end justify-between mb-12 gap-6 flex-wrap">
          ...
          <Link to="/announcements" className="text-sm font-medium text-primary hover:text-accent inline-flex items-center gap-1">All news <ArrowRight className="h-4 w-4" /></Link>
        </div>
      ...
      <section className="bg-primary text-primary-foreground">
        <div className="mx-auto max-w-7xl px-6 lg:px-10 py-16 grid gap-6 md:grid-cols-3 text-center">
          {[
            ...
            <Link key={q.to} to={q.to} className="group block py-6 px-4 rounded-2xl hover:bg-primary-foreground/5 transition-colors">
              <h3 className="font-display text-2xl group-hover:text-accent transition-colors">{q.label}</h3>
