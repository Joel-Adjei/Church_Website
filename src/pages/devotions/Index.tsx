          ...
          {isLoading ? (
            ...
            <div className="mt-6">
              <h1 className="font-display text-4xl md:text-6xl leading-[1.05]">
                A quiet word is on the way
              </h1>
              <p className="mt-4 text-lg text-primary-foreground/80 max-w-2xl">
                We're preparing today's devotion. Check back soon.
              </p>
            </div>
          ) : (
            <>
              ...
              <div className="mt-10 flex flex-wrap gap-3">
                ...
                <ShareButtons
                  title={today.title}
                  excerpt={today.Bible_verse.verse_content}
                  url={
                    typeof window !== "undefined"
                      ? `${window.location.origin}/devotions/${today.id}`
                      : undefined
                  }
                  align="start"
                />
              ...
              {shown.map((d) => (
                ...
                >
                  ...
                  <div className="p-6 flex-1 flex flex-col">
                    ...
                    <h3 className="mt-3 font-display text-xl text-ink leading-snug group-hover:text-primary">
                      {d.title}
                    </h3>
                    <p className="mt-2 text-sm italic text-ink-muted">
                      "{d.Bible_verse.verse_content.slice(0, 100)}
                      {d.Bible_verse.verse_content.length > 100 ? "…" : ""}"
                    </p>
                  </div>
                </Link>
              ))}
